...
> **かんたん解説（はじめての方へ）**
> LLMesh は、いろいろな AI（文章を作る人工知能）を 1 か所にまとめて呼び出せる「受付係」のような道具です。会社の機密や個人の情報を外に出さず、自分のパソコンや社内のサーバーの中だけで AI を動かせるように作られています。工場のセンサー（温度や振動を測る装置）の数字を見張って、ふだんと違う動きが出たら AI に「なぜおかしいか」を説明させる、といった使い方もできます。
> 知らない言葉が出てきたら [用語集（GLOSSARY.md）](GLOSSARY.md) を見てください。
