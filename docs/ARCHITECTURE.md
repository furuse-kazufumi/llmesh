...
> **かみ砕いた説明（中学生レベル）**
> このページは、LLMesh という仕組みの「設計図」を説明したものです。家の中だけにある何台かのAI（外に情報を出さないAI）を、共通の通話ルール（MCP）でつないで、コードを書く・点検する・テストを作る作業を手分けして進めます。
> 大事な考え方は「あやしい入力は全部止める」「秘密の情報は外に送らない」「誰が何をしたか必ず記録に残す」の3つです。これらを最初から設計の土台に組み込んでいます。
> 分からない言葉が出てきたら → [用語集（GLOSSARY.md）](GLOSSARY.md) を見てください。
...
├── test_domains_materials.py    # domains.materials predictor + discover_top_k (Phase 4)
├── test_rag_parsers.py          # document parsers: text/md/html/pdf (Phase 5)
├── test_rag_multimodal.py       # multimodal memory + cosine + facade (Phase 5)
