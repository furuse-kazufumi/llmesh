...
### F-6.7 ガバメントAI「源内」バックエンド（2026-05-11 追加 — v3.2+）

デジタル庁が全府省庁約 18 万人の国家公務員向けに展開するガバメントAI
「源内（Genai）」（2026-04-24 OSS 公開）を LLMesh の公式バックエンドとして
サポートする。産業 IoT × 行政データ融合ユースケース（スマートシティ、
インフラ監視レポート、行政文書 RAG 等）を主な対象とする。

#### F-6.7.1 GenaiBackend（OpenAI 互換ラッパー）

- `GenaiBackend` — `OpenAICompatibleBackend` を継承し、源内固有のエンドポイント
  / 認証方式を吸収する薄いラッパー
- エンドポイント: `GENAI_ENDPOINT_URL` 環境変数で設定（デフォルト未設定 =
  接続拒否）
- 認証: `GENAI_API_KEY` 環境変数（Bearer ヘッダ）または
  `GENAI_API_KEY_HEADER` で任意ヘッダ名を上書き可能
- 国産 LLM モデル選択（`model=` 引数で指定）:
  - NTTデータ `tsuzumi-7b` / `tsuzumi-70b`
  - NEC `cotomi-pro` / `cotomi-light`
  - 富士通 `Takane` シリーズ
  - Preferred Networks (PFN) `PLaMo-100B`
  - その他 F-6.1 `OpenAICompatibleBackend` 互換モデル名は全て受け入れ

#### F-6.7.2 行政 RAG テンプレート統合

- **F-6.7.2a** 法制度 RAG ブリッジ — 源内公式テンプレート（法律条文参照 AI
  / Google Cloud ベース）を LLMesh `RAGPipeline` から呼び出すアダプタ
- **F-6.7.2b** 行政文書チャンク戦略 — 公文書・告示・通達の
  `DocumentChunker` プリセット（条文番号 → SensorEvent `payload.clause_id`
  として伝搬）
- **F-6.7.3** `llmesh genai init` — 環境変数テンプレート
  `.env.genai.example` を生成する CLI ワンライナー

#### F-6.7.4 プライバシー・セキュリティ不変条件

- **F-6.7.4a** PromptFirewall L4 必須: 個人番号（マイナンバー）/ 住所 /
  行政機密フラグが含まれるプロンプトは **BLOCK**（既存 L4 ルール拡張で対応）
- **F-6.7.4b** `EndpointValidator` に `GENAI_ENDPOINT_URL` を許可リストへ
  追加し、SSRF を遮断
- **F-6.7.4c** 監査ログに `genai_model` / `genai_invoke` イベントを記録。
  プロンプト本文は SHA-256 のみ（行政情報の外部漏洩防止）
- **F-6.7.4d** 回線断フォールバック: `GenaiBackend` が到達不能の場合は
  `OllamaBackend`（国産モデル tsuzumi / PLaMo のローカル量子化版）へ
  自動切替（`circuit_breaker` 既存機構を流用）

#### F-6.7.5 マルチクラウド構成対応

- AWS（行政 RAG テンプレート）/ Azure（セルフデプロイ LLM テンプレート）/
  Google Cloud（法制度 AI）の 3 クラウドすべてで `GenaiBackend` が動作
- デプロイ構成は `GENAI_CLOUD=aws|azure|gcp` 環境変数で切替可能
- 既存 `AdapterCircuitBreakerRegistry` / `read_capped` / `OutputValidator`
  と完全互換

#### F-6.7.6 受入基準

- `GenaiBackend` が `LLMBackend` ABC テスト suite を全 PASS
- モック化した源内エンドポイントで認証 / チャンク / RAG 呼び出しが成功
- L4 PromptFirewall でマイナンバーパターンが BLOCK される回帰テスト
- 国産 LLM 4 モデル名（tsuzumi / cotomi / Takane / PLaMo）を model 引数で
  渡したとき正しい request body が生成される

#### F-6.7.7 参考リンク（2026-04-24 時点）

- OSS: デジタル庁 GitHub 公式リポジトリ（Apache-2.0 / CC BY 4.0）
- 公式案内: https://www.digital.go.jp/en/policies/genai
...
| ★★ | **F-6.7 ガバメントAI 源内バックエンド** | **行政 × 産業 IoT 融合・国産 LLM 公式サポート（v3.2+）** |
