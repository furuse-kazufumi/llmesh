...
def register_adapter(name: str, adapter_cls: type) -> None:
    """Register a custom ProtocolAdapter for use with FanoutExecutor.

    Convenience wrapper around AdapterRegistry.register()::

        from llmesh.orchestrator.fanout import register_adapter
        register_adapter("grpc", MyGRPCAdapter)
        executor = FanoutExecutor(protocol="grpc")
    """
...
class FanoutExecutor:
    ...
    Args:
        ...
        protocol:         Transport protocol for NodeClient: "http" (default), "tcp",
                          "udp", or any name registered in AdapterRegistry.
                          "http" preserves all existing behaviour unchanged.
        fairness_policy:  Optional FairnessPolicy — when no selector is attached,
                          BLOCKED/EXCLUDED nodes are dropped before sending.
                          When a selector is attached, fairness filtering is
                          handled by the selector itself.
    """
    ...
    def __init__(
        ...
        self._node_client = NodeClient(timeout=node_timeout, protocol=protocol)
    ...
    def execute(
        ...
        *,
        ...
        if is_expired(body.get("deadline")):
            raise DeadlineExpiredError(f"fanout request deadline expired before dispatch: tool={tool_name}")
        ...
        # Fairness pre-filter when no selector (selector handles it internally when present)
        if self._fairness_policy is not None and self._selector is None:
            candidates = [
                n for n in candidates if self._fairness_policy.is_allowed(n.node_id)
            ]
