...
> **かみ砕いた説明（中学生レベル）**
>
> このページは、ソフトを古い版から新しい版へ入れ替えるときの「引っ越しの手引き」です。
> 部屋の模様替えで「この家具はそのまま使える」「この箱は新しい棚に移してね」と案内するように、どの設定や書き方をそのまま残せて、どこだけ直せばよいかを版ごとにまとめています。
> 大きな番号が変わるとき（たとえば二の位から三の位へ）は、思わぬ食い違いを避けるため、ここを先に読んでから入れ替えると安全です。
>
> 用語の意味は [用語集（GLOSSARY.md）](./GLOSSARY.md) にまとめてあります。
