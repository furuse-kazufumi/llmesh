"""Tests for the RepIR JSON Schema and MCP result builder."""
...
from llmesh.repir.schema import REPIR_DOCUMENT_SCHEMA
...
def test_document_validates_against_schema():
    jsonschema.validate(_doc().to_dict(), REPIR_DOCUMENT_SCHEMA)


def test_schema_rejects_unknown_node_type():
    bad = {"repSchema": "repir/0.1", "root": {"type": "wormhole"}}
    with pytest.raises(jsonschema.ValidationError):
        jsonschema.validate(bad, REPIR_DOCUMENT_SCHEMA)
...
def test_output_schema_wraps_under_repir():
    out_schema = repir_output_schema()
    payload = {"repIR": _doc().to_dict()}
...
def test_build_result_has_structured_and_text():
    ...
    # Typed tree in structuredContent under repIR, schema-valid.
    assert "repIR" in result["structuredContent"]
    jsonschema.validate(result["structuredContent"], repir_output_schema())
...
def test_oversize_structured_content_degrades_to_text_only():
    ...
    assert result["_meta"]["repir.structured_omitted"] is True
