...
> **Historical snapshot.**
> このファイルは 2026-05-05 時点の特定セッションを記録したスナップショットです。
> 数値（テスト総数 448 passed など）はその時点のものであり、**現行値ではありません**。
> 最新のテスト件数・ステータスは `README.md` および `ARCHITECTURE.md`、ロードマップ
> 状況は `docs/ROADMAP.md` を参照してください（現在 526 passed / 0 failed）。
